const roomName ="⚡ THE ULTIMATE FUTSAL 3V3 ⚡";
const botName = "CPOL";
const maxPlayers = 20;
...
var redTeamArray = [];
var blueTeamArray = [];
//añado aqui el record exacto para añadirlo manualmente
var streakRecord = 0;
var streakRecordPlayers = "";  
var streakmanualRecord = 0; //STREAKSEMANAL SIEMPRE
var streakRecordPlayersmanual = "NO DEFINIDO"; //STREAKSEMANAL SIEMPRE
...
function currentStreakFun(player) { // !currentStreak
	if (streakpuntua > 0){
		...
		room.sendAnnouncement("No se han establecido rachas todavía. Jugar algunos juegos primero.", player.id, 0xf7d600, 'normal', 0);
	}
	return false;
}
...
function streakRecordFun(player) { // !recordStreak
	...
	room.sendAnnouncement("No se han establecido records todavía. Jugar algunos juegos primero.", player.id, 0x00FF00, 'normal', 0);
	}
	else{
	room.sendAnnouncement("No se han establecido records todavía. Jugar algunos juegos primero.", player.id, 0x00FF00, 'normal', 0);	
	}
...
function endGame(winner) { // handles the end of a game : no stopGame function inside
	...
	if (winner == Team.RED) {
		if (scores.red == scores.blue){
			...
			streakpuntua++;
		    room.sendChat("🔴 El Red Team ganó " + scores.red + "-" + scores.blue + " ! Victorias consecutivas : " + streakpuntua + " 🏆");	
			}else if(players.length < 2 * maxTeamSize){
			streakpuntua = 0;
		    room.sendChat("🔴 El Red Team ganó " + scores.red + "-" + scores.blue + " ! Victorias consecutivas : 1" + " 🏆");	
			}
		}else{
			streak++;
		    if(players.length >= 2 * maxTeamSize){
		...
		if (scores.red == scores.blue){
			...
			streakpuntua = 1;
            room.sendChat("🔵 El Blue Team ganó " + scores.blue + "-" + scores.red + " ! Victorias consecutivas : " + streakpuntua + " 🏆");
			}else if(players.length < 2 * maxTeamSize){
			streakpuntua = 0;
		    room.sendChat("🔵 El Blue Team ganó " + scores.blue + "-" + scores.red + " ! Victorias consecutivas : 1"  + " 🏆");	
			}
		}else{
		    streak = 1;
		    if(players.length >= 2 * maxTeamSize){
    ...
    scores.red == 0 ? (scores.blue == 0 ? room.sendChat("🏆 " + GKList[0].name + " y " + GKList[1].name + " mantuvieron la portería invicta ! ") : room.sendChat("🏆 " + GKList[1].name + " mantuvo la portería invicta ! ")) : scores.blue == 0 ? room.sendChat("🏆 " + GKList[0].name + " mantuvo la portería invicta ! ") : null;
	updateStats();
...
function updateRoleOnPlayerIn() {
	...
	if (inChooseMode) {
		...
		if (teamS.length > 0) {
		getSpecList(teamR.length <= teamB.length ? teamR[0] : teamB[0]);//getSpecList		
		}
	}
	balanceTeams();
}
function updateRoleOnPlayerOut() {
		...
		if (players.length >= 2 * maxTeamSize && scores.time >= (0/12) * game.scores.timeLimit && teamR.length != teamB.length) {
			if (teamR.length < teamB.length) {	
			    ...
			    if (scores.blue - scores.red == 2) {
					...
					setTimeout(() => { room.stopGame(); }, 1);//100
					return;
				}
			}
			else {
	...
	if (inChooseMode) {
		...
		if (Math.abs(teamR.length - teamB.length) == teamS.length) {
			...
			if (teamR.length > teamB.length) {
				for (var i = 0 ; i < b ; i++) {
					setTimeout(() => { room.setPlayerTeam(teamS[0].id, Team.BLUE); }, 1);//1*i
				}
			}
			else {
				for (var i = 0 ; i < b ; i++) {
					setTimeout(() => { room.setPlayerTeam(teamS[0].id, Team.RED); }, 1);//1*i
				}
			}
			return;
		}
		...
		capLeft ? choosePlayer() : getSpecList(teamR.length <= teamB.length ? teamR[0] : teamB[0]);//getSpecList
	}
	balanceTeams();
}
...
function choosePlayer() {
	...
	if (teamR.length <= teamB.length && teamR.length != 0) {
		room.sendAnnouncement("[PV] Para elegir un jugador, ingrese su número en la lista dada o use 'top', 'random' o 'bottom'.", teamR[0].id,0xE033FF,"normal",2);
		timeOutCap = setTimeout(function (player) { room.sendAnnouncement("[PV] Date prisa @" + player.name + ", solo quedan " + Number.parseInt(chooseTime / 2) + " segundos para elegir !", player.id, 0xE033FF, "normal", 2); timeOutCap = setTimeout(function (player) { room.kickPlayer(player.id, "¡No elegiste a tiempo!", false); }, chooseTime * 500, teamR[0]); }, chooseTime * 1000, teamR[0]);
	}
	else if (teamB.length < teamR.length && teamB.length != 0) {
		room.sendAnnouncement("[PV] Para elegir un jugador, ingrese su número en la lista dada o use 'top', 'random' o 'bottom'.", teamB[0].id, 0xE033FF, "normal",2);
		timeOutCap = setTimeout(function (player) { room.sendAnnouncement("[PV] Date prisa @" + player.name + ", solo quedan " + Number.parseInt(chooseTime / 2) + " segundos para elegir !", player.id, 0xE033FF, "normal", 2); timeOutCap = setTimeout(function (player) { room.kickPlayer(player.id, "¡No elegiste a tiempo!", false); }, chooseTime * 500, teamB[0]); }, chooseTime * 1000, teamB[0]);
	}
	if (teamR.length != 0 && teamB.length != 0) getSpecList(teamR.length <= teamB.length ? teamR[0] : teamB[0]);//getSpecList
}
...
setInterval(() => {
    if (statNumber % 5 == 0) showRankings(Ss.GA, "Partidos Jugados");
    if (statNumber % 5 == 1) showRankings(Ss.WI, "Victorias");
    if (statNumber % 5 == 2) showRankings(Ss.GL, "Goles");
    if (statNumber % 5 == 3) showRankings(Ss.AS, "Asistencias");
    if (statNumber % 5 == 4) showRankings(Ss.CS, "CS");
    statNumber++;
}, statInterval * 60 * 1000);

function TakimMesajlasmasi(teammesaj, oyuncuismi, takimid) {
  for (
    var index = 0; index < room.getPlayerList().filter((p) => p.team == takimid).length; index++
  ) {
	  //room.sendAnnouncement("[Spec] " + oyuncuismi + ": " + teammesaj, teamPlayer.id, 0xdee7fa, "normal", 1);

	  if(takimid == 0){
	  room.sendAnnouncement(
      "[Spec] " + oyuncuismi + ": " + teammesaj,
      room.getPlayerList().filter((p) => p.team == takimid)[index].id, 0xdee7fa, "bold", 1
    );
	  }
	  else if(takimid == 1){
	  room.sendAnnouncement(
      "[Team] " + oyuncuismi + ": " + teammesaj,
      room.getPlayerList().filter((p) => p.team == takimid)[index].id, 0xED6A5A, "bold", 1
    );
	  }
	  else if(takimid == 2){
	  room.sendAnnouncement(
      "[Team] " + oyuncuismi + ": " + teammesaj,
      room.getPlayerList().filter((p) => p.team == takimid)[index].id, 0x5995ED, "bold", 1
    );
	  }
...
room.onPlayerChat = function (player, message) {
	var mensajenosplit = message;
	message = message.split(/ +/);
	
	if (mensajenosplit.startsWith("@")) 
	{
        splitSpot = mensajenosplit.indexOf('_');
		splitSpace = mensajenosplit.indexOf(' ');
		
		if(splitSpot == -1)
			var nick = mensajenosplit.substr(1,splitSpace - 1);
		...
		var tempPlayer = room.getPlayerList().filter((player) => player.name == nick);
		if(tempPlayer[0] != undefined)
			toID = tempPlayer[0].id;
		msgToSend = mensajenosplit.substr(splitSpace + 1);
        
        
        if (tempPlayer.length != 0) 
		{
            fromName = player.name;
            toName = tempPlayer[0].name;
            //sentResp = '[📤 PM to ' + toName + '] ' + msgToSend; "[PM > " + toName + "] " + fromName + ": " + msgToSend
            //recvResp = '[📥 PM from ' + fromName + '] ' + msgToSend; "[PM] " + fromName + ": " + msgToSend
            sentResp = "[PM > " + toName + "] " + fromName + ": " + msgToSend;
			recvResp = "[PM] " + fromName + ": " + msgToSend;
            //room.sendChat(sentResp, player.id);
            //room.sendChat(recvResp, toID);
			room.sendAnnouncement(sentResp, player.id, 0xff20ff, "bold", 1);	
			room.sendAnnouncement(recvResp, toID, 0xff20ff, "bold", 1);
            return false;
        }
    }
	if (mensajenosplit.startsWith("t ")) {
    var tdmmesaj = mensajenosplit.substring(1).trim();
    TakimMesajlasmasi(tdmmesaj, player.name, player.team);
    return false;
  }
	player.team != Team.SPECTATORS ? setActivity(player, 0) : null;
	if (["!help"].includes(message[0].toLowerCase())) {
	room.sendChat("[PV] Comandos : !me, !games, !wins, !goals, !assists, !cs, !afks, !mutes, !bans, !racha, !record.", player.id);
	room.sendChat("[PV] Commands : t mensaje, @id mensaje", player.id)
	...
	/*else if (["!games"].includes(message[0].toLowerCase())) {
		...
		tableau.sort(function (a, b) { return b[1] - a[1]; });
		room.sendChat("[PV] Porterías Invictas> #1 " + tableau[0][0] + ": " + tableau[0][1] + " #2 " + tableau[1][0] + ": " + tableau[1][1] + " #3 " + tableau[2][0] + ": " + tableau[2][1] + " #4 " + tableau[3][0] + ": " + tableau[3][1] + " #5 " + tableau[4][0] + ": " + tableau[4][1], player.id);
	}*/
	  else if (["!games"].includes(message[0].toLowerCase())) {
        showRankings(Ss.GA, "Partidos Jugados", player);
    }
    else if (["!wins"].includes(message[0].toLowerCase())) {
        showRankings(Ss.WI, "Victorias", player);
    }
    else if (["!goals"].includes(message[0].toLowerCase())) {
        showRankings(Ss.GL, "Goles", player);
    }
    else if (["!assists"].includes(message[0].toLowerCase())) {
        showRankings(Ss.AS, "Asistencias", player);
    }
    else if (["!cs"].includes(message[0].toLowerCase())) {
        showRankings(Ss.CS, "CS", player);
    }
	else if (["!currentStreak", "!racha"].includes(message[0].toLowerCase())) {
      currentStreakFun(player);
    }
	else if (["!recordStreak", "!record"].includes(message[0].toLowerCase())) {
      streakRecordFun(player);
    }
...
room.onTeamGoal = function(team) {
	...
	const scores = room.getScores();
	game.scores = scores;
	if(lastPlayersTouched[0] != undefined){
	...
	if (scores.scoreLimit != 0 && (scores.red == scores.scoreLimit || scores.blue == scores.scoreLimit && scores.blue > 0 || goldenGoal == true)) {
		endGame(team);
		goldenGoal = false;
		setTimeout(() => { room.stopGame(); }, 10);//1000
	}
...
setTimeout(() => { 
    setInterval(() => {
		if(streakpuntua > 0 && !inChooseMode && streakRecord >= 0){
		room.sendAnnouncement("🏆 Racha: " + streakpuntua + ", Jugadores: " + redPlayers + " 🏆", null, 0xf7d600, 'normal', 0);
	    if(streakmanualRecord >= streakRecord){
		room.sendAnnouncement("🏆 Record: " + streakmanualRecord + ". Jugadores: " + streakRecordPlayersmanual + " 🏆",null, 0x00FF00, 'normal', 0);
		}
		else if(streakRecord > streakmanualRecord){
		room.sendAnnouncement("🏆 Record: " + streakRecord + ". Jugadores: " + streakRecordPlayers + " 🏆",null, 0x00FF00, 'normal', 0);			
		}
	room.sendAnnouncement("🏆 Si quereis ver la racha o el record de la sala son los comandos !racha y !record" + " 🏆", null, 0xf7d600, 'normal', 0);
			}
}, 31 * 60 * 1000); }, 10000);

setTimeout(() => {
	setInterval(() => {
		room.sendAnnouncement("⚽ Este servidor está configurado por Gimenez20#5378 ⚽", null, 0xE6E6FA, 'normal', 0);
		room.sendAnnouncement("⚽ Ofrecemos la posibilidad de crear salas 24/7 operativas a un precio bajo, más info por PM ⚽", null, 0xE6E6FA, 'normal', 0);
		room.sendAnnouncement("⚽ También podemos configurar códigos para vuestras salas publicas/privadas ⚽", null, 0xE6E6FA, 'normal', 0);
	}, 39 * 60 * 1000);
}, 10000);
